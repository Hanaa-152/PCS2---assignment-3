...
def solve_ctea(file_content):
    sequences = [str(record.seq) for record in SeqIO.parse(StringIO(file_content), "fasta")]
    s, t = sequences[0], sequences[1]
    edit_distance, aligned_s, aligned_t = ctea(s, t)
    return edit_distance, aligned_s, aligned_t

with open("/Users/han3/Downloads/rosalind_ctea-7.txt", "r") as file:
    file_content = file.read()

edit_distance, aligned_s, aligned_t = solve_ctea(file_content)
print(edit_distance)
print(aligned_s)
print(aligned_t)
